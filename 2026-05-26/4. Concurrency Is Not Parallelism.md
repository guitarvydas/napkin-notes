...
- **TL;DR + L;R structure** added, with the thesis (synchronous functions cannot express async) up front
- **Opening claim fixed** — "used interchangeably in casual English" rather than "mean essentially the same thing"
- **"Multi-Threading Fakes Parallelism"** and **"True Parallelism Requires Dedicated CPUs"** replace the thin noun headings
- **Struct analogy payoff** made explicit: _same phenomenon, different dimension (space vs time)_
- **"Squeezing blood"** replaced with a single clean image
- **Cartridge paragraph** reintegrated at the end of the PBP section with an explicit bridge back to 0D's core idea: isolated components, explicit wiring, no shared memory
- **Grace Hopper quote** pulled inline
- **"Let's begin / Let's examine"** invitation framing removed throughout
...
We continue using old-fashioned shared-memory ideas because, as Grace Hopper put it, "we've always done it this way" (https://programmingsimplicity.substack.com/p/capt-grace-hopper-on-future-possibilities?r=1egdky).
...
In the early days of stand-alone computers, we chose small-grained memory sharing over message passing. That choice produced accidental complexity whose most visible symptom is "thread safety". The problems of thread safety evaporate when isolated computers are used. Programming complexity drops when thread safety no longer inflicts itself on thought patterns, language design, and operating system architecture.
